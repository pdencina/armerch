...
const fmtDate = (d: string) =>
  new Intl.DateTimeFormat('es-CL', { day:'2-digit', month:'short', year:'numeric', hour:'2-digit', minute:'2-digit' }).format(new Date(d))
...
export default function MovementsPage() {
  ...
  useEffect(() => {
    ...
    supabase.from('inventory_movements')
      .select(`id, type, quantity, notes, created_at, product:products(name, sku), created_by_profile:profiles(full_name)`)
      ...
      <div className="bg-zinc-800/30 rounded-xl border border-zinc-700/40 overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {movements.map(m => {
                const cfg = TYPE_CONFIG[m.type as keyof typeof TYPE_CONFIG] ?? TYPE_CONFIG.ajuste
                const Icon = cfg.icon
                return (
                  <tr key={m.id} className="border-b border-zinc-700/30 hover:bg-zinc-700/20 transition">
                    <td className="px-4 py-3 text-xs text-zinc-500 whitespace-nowrap">{fmtDate(m.created_at)}</td>
                    ...
                    <td className="px-4 py-3">
                      <span className={`inline-flex items-center gap-1.5 text-[10px] font-semibold px-2 py-1 rounded-full ${cfg.bg} ${cfg.color}`}>
                    ...
                    <td className="px-4 py-3 text-xs text-zinc-500">{m.created_by_profile?.full_name ?? '—'}</td>
          ...
          {movements.length === 0 && <div className="py-14 text-center"><p className="text-zinc-600 text-sm">Sin movimientos</p></div>}
        </div>
      </div>
    </div>
  )
